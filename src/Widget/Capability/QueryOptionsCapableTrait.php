...
trait QueryOptionsCapableTrait {
  ...
  /**
   * Turn the widget's rows over to a query source.
   *
  ...
  /**
   * Take on a resolved query's rows as the widget's own candidates.
   *
   * @param list<\DrevOps\Tui\Model\Option> $rows
   *   The rows.
   */
  ...
  protected function queryStateLine(ThemeInterface $theme): ?string {
    ...
    if (Strings::length($this->query()) < $this->queryMinLength) {
      return $theme->description(Translator::formatPlural($this->queryMinLength, 'Type 1 character to search.', 'Type @count characters to search.'));
    }
