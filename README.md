...
| 🧭 Layouts | `->layout('two-column')` arranges a screen or a panel into named regions, each with its own size, flow, scrolling and edges; `->place()` puts a block of your own in one and `->flow()` turns it across; `->inspect()` boxes and captions every region at every depth while you write one | [layouts](https://phptui.dev/layouts) | [`20-layouts-*`](playground) |
