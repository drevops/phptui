...
Four kinds of key never reach the router, and all four for the same reason - none of them acts on a block. Pressing a button ends the form or closes the dialog it belongs to; activating a `Progress` row runs its work against the terminal a step at a time, repainting between steps; leaving is about the session rather than about anything in it; and the wheel moves the surface the cursor is on by a row - the `Region` it is in, or the arrangement itself where that is what moves as one - leaving the cursor where it was until the next key that could move it. A block never learns where it is drawn, so whoever owns the terminal holds these.
