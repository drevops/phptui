...
A note body can carry a `[text](url)` link, and with [`->markdown()`](/markdown) on it also renders bold, emphasis, inline code and bullet lists - all degrading to clean plain text where the terminal cannot show them.
