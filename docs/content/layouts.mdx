...
Each window scrolls its own region, as any region does when its contents outrun it, and the visual rows themselves are apportioned by the layout. A row you write on a grid-arranged panel goes in `content` and is drawn above the windows.
