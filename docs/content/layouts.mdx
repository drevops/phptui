...
### Seeing every region at once

`->inspect()` on the facade boxes and captions **every** region, at every depth - the screen's, the panel drawn in it, and every panel below that:

```php
(new Tui($form))->layout('market-hall')->inspect()->run();
```
...
It is a development tool rather than part of a form's design: reach for it while writing a layout, to see which region a row actually landed in and how much of the axis each one was granted. A region that declared `->outlined()` keeps its own caption; the rest are captioned with their names.

The boxes cost what a declared box costs, so an inspected frame holds less than the frame it describes - the rows sit where the boxes leave room rather than where they will ship.
...
The [standard furniture](#where-the-standard-furniture-goes) is placed first, so a block of yours lands **after** the trail or the key hints its region already holds. `tail: TRUE` packs it from the end of the region's run instead of the start, which is the same statement `->tail()` makes on a region.
...
Everything the session places goes into the **screen's** regions, never into the panels the form declared - so a second run over the same declaration opens on the same first frame rather than on the last run's furniture. That holds for [blocks you place yourself](#putting-your-own-blocks-in-a-region) too. A layout meant to arrange a panel's own blocks can name its regions anything, because nothing is placed in it but what the form places itself.
