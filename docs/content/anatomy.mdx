---
title: Anatomy
description: 'What a screen is made of, piece by piece: the window chrome, a field in both of its modes, and the theme element that draws each one.'
...
An **atom** is one named piece of that interface - the smallest thing worth naming on its own, and the unit a [theme](/themes) restyles. Every atom is drawn by an **element**, the method a theme answers with, and [Elements](#elements-what-a-theme-actually-implements) lists all of them - not just a field's, but every block's. This page is what there is to draw; [Themes](/themes) is how to draw it.
...
Two routes reach the elements above, and which one you want depends on how much you are restating. [Writing a theme](/themes#writing-a-theme) covers the subclass: repaint a whole family of them through the voices behind them, or override a single element outright. [Patching an element](/themes#patching-an-element) covers the closed set of nine a closure can restate without a class at all.

Which of them a theme can answer for is its own declaration. [What a theme is allowed to do](/themes#what-a-theme-is-allowed-to-do) is the whole set of six, and a theme that declares none of them still draws every element on this page - in the strings it was handed and the stand-ins that read without color or glyphs.
