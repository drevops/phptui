...
A card and a grid both go to the theme's card renderer - the same one behind [`card()` and `table()`](/output) on the output primitive - so restyling a standalone card restyles an in-form one with it.
