...
Each of its windows is a _region_ of its own, so `GridLayout(1, 2)` declares four: `content`, which holds the panel's own rows above the grid, and then `window-1`, `window-2` and `window-3`, numbered in reading order - across the first visual row, then across the second. A _panel_ reaches a window by naming that region, exactly as any block reaches any other, and is drawn there as a **window** - a column showing what is behind it rather than a row summarizing it. That is the region's declaration, not the panel's: a row has one line to say what is behind a panel and a window has the depth to show it, so which of the two the space is can only be answered by the arrangement.
...
Each window **scrolls its own region**, as any region does when its contents outrun it. The visual rows themselves are apportioned by the layout, which is the only thing that sees them all, and the driver's follow-the-cursor rule works unchanged: the cursor sits on a window rather than inside one, so the region holding it is already showing the row the cursor is on.
