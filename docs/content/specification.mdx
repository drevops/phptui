...
A surface also _does_ its own scrolling. A region is handed one number - the size it was given - and everything after that is its own work: how tall its blocks add up to, where the viewport sits, whether an overflow marker is due, and how the cursor or the wheel moves it. An arrangement answers the same questions about the lines it stacks. No sibling is involved either way, which is what makes _Scroll_ the surface's own capability rather than something apportioned to it.
