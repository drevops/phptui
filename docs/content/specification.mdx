...
`AbstractLayout` answers with the conventional names - `header`, `content`, `footer` - so every layout that uses them is furnished without writing a line. A layout that calls its regions something else overrides it and says where each piece goes; one that answers `NULL` keeps that piece off the screen entirely. Only `Furniture::Body` has to be answered: a layout with nowhere to draw the form is refused where the layout is named, because there is nothing to fall back to.
...
The region also _does_ the scrolling. Its layout hands it one number - the size it was given - and everything after that is the region's own work: how tall its blocks add up to, where the viewport sits, whether an overflow marker is due, and how the cursor or the wheel moves it. No sibling is involved in any of that, which is what makes _Scroll_ the region's capability rather than the layout's.
...
The refusal is the block's own row rather than a note somebody draws above it, and it sits against the buttons with nothing between them - not even the blank row the theme puts between one block and the next. A message that could drift a row away from what it refuses is a message about nothing.
...
| `ActionsElementsInterface`    | the buttons, the gap between them, and the reason one is withheld    |
...
Four kinds of key never reach the router, and all four for the same reason - none of them acts on a block. Pressing a button ends the form or closes the dialog it belongs to; activating work runs it against the terminal a step at a time; leaving is about the session rather than about anything in it; and the wheel moves the _region_ the cursor is in, which is not a block at all. `ScreenController` holds those, because a panel knows about none of them and a block never learns where it is drawn.

The wheel is the clearest of the four. It moves the viewport by a row and leaves the cursor exactly where it was, so a reader can look ahead without answering anything - and the moment they press a key that could move the cursor, the rule that keeps the cursor in sight wins the viewport back. A region that was not declared to _Scroll_ ignores it, because there is nothing to move through.
...
**Can a region be addressed from outside its layout?** A block goes in by name, and the name is the layout's - so whatever places blocks asks the layout where each piece belongs rather than going looking for a name it hopes is there. `furnishes()` is that question, asked one role at a time. `default` answers `header`, `content` and `footer`; `two-column` answers `left` for the form and nothing for the trail or the keys, so neither is drawn; a layout of your own answers whatever it likes, and a piece it refuses is a piece it keeps off the screen. Only the form itself has to be answered, because there is nothing to draw it in place of. The trail and the keys keep tracking the session either way - they are just never drawn.
