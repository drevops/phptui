...
| Interface                     | Draws                                                                |
| ----------------------------- | -------------------------------------------------------------------- |
| `ChromeElementsInterface`     | the border, and the mark saying a region's contents outran it        |
...
**Can a whole panel depend on an answer?** _Depend_ is claimed by a _field_, by _markup_ and by a _progress_ row, so three of the seven blocks can come and go with the answers. A _panel_ is not among them, so a whole section cannot appear or disappear on an earlier answer - only the blocks inside it can, one at a time.
