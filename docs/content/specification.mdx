...
| `->outlined('Notes')`   | a box around it, captioned in the top edge                |
...
`outlined()` spends the region's own cells rather than asking for more: a row top and bottom and a column each side of what the layout granted, the way the frame around the whole screen spends the terminal's. Siblings are untouched, because the box is drawn inside the grant. Below three rows or three columns there is no room for a box and any contents both, so the region draws its contents and no edge. The facade's `inspect()` turns one on for every region at every depth, captioned with its name, which is a development tool rather than part of a form's design.
