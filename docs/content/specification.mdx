...
final class SidebarLayout extends AbstractLayout {
  ...
  public function __construct() {
    ...
    $this->region('delivery')->flex(1)->scrolls();
  }
...
```php
$p->panel('Certification', function (PanelBuilder $sp): void {
  $sp->when(new Condition('organic_only', eq: TRUE));
  $sp->text('Certifier');
});
```
...
```php
$p->select('Basket contents')                      // Show, Focus
  ...
  ->when(new Condition('organic_only', eq: TRUE)); // Depend
```
...
$p->markup('Deliveries leave at dawn.')->border();

// The same block again, laid out as a table under a title.
$p->markup('', 'Yields per crate', 'yields')
...
```php
$p->markup('Organic crates need current certification.')
  ->when(new Condition('organic_only', eq: TRUE));
```
...
$form = Form::create('Orchard')
  ->panel('Delivery', function (PanelBuilder $p): void {
    $p->text('Courier');
    $p->number('Basket weight')->min(200)->max(9000);
...
->panel('Delivery', function (PanelBuilder $p): void {
  $p->layout('two-column');
  $p->in('left')->text('Courier');
  $p->in('right')->number('Basket weight');
})
```
