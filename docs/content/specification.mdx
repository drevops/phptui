...
Where that line falls is worth stating outright, because it's the rule that keeps elements reusable. **An element takes plain scalars and nothing else** - strings, integers, booleans - so nothing handed to one can carry a field, a panel or an answer set in behind it. The theme-wide methods on `ThemeInterface` are the one place that widens, and only because they belong to no block: `keyGlyph()` takes a `Key`, an input value object, because writing a key is vocabulary rather than styling and the key press is the thing being written. A value object describing input is fair game there. Form state isn't, anywhere.
...
| Theme        | `ThemeInterface` (two methods), `AbstractTheme` as the floor, `DefaultTheme` above it, the six `*CapableInterface` declarations with `ColorSchemeCapableTrait` and `UnicodeCapableTrait`, and `ThemeManager` to name any of them                                                                                                                                                                                   |
