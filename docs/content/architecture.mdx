...
Four kinds of key never reach the router, and all four for the same reason - none of them acts on a block. Pressing a button ends the form or closes the dialog it belongs to; activating a [progress row](/progress-row) runs its work against the terminal a step at a time, repainting between steps; leaving is about the session rather than about anything in it; and the wheel moves the `Region` the cursor is in, which is not a block at all. A block never learns where it is drawn, so whoever owns the terminal holds these.
