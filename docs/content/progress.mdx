...
A loader resolves one list, once. When the candidates are too many to hold - a catalogue behind a search API, a database lookup, an index - a [search](/widgets/search) or [suggest](/widgets/suggest) field can source them from the query instead, with `->optionsFrom()`:
...
The panel loop is synchronous, so the call blocks: the field paints a themed `Loading…` in place of its list, then repaints with the result. Three behaviours keep that from becoming a call per keystroke:
