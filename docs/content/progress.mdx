...
`progress()` runs _around_ the form. Three counterparts show feedback _inside_ the interactive panel, drawn by the same theme and resolved the first time a panel opens - so a drill-in fetches only what that panel needs, once.
...
- **Preloading a panel.** `->preload(callable)` on a panel runs once, before the panel's fields first draw - prep the panel needs, fetched on entry rather than up front, so one fetch can feed several fields.
