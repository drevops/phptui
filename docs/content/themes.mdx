...
To restyle one piece outright rather than recolor a family, override its **element** - the public method the block asks for. [Anatomy](/anatomy#elements-what-a-theme-actually-implements) lists every one of them, grouped by the block that declares it:
