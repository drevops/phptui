---
title: Themes
description: 'Six built-in themes selectable by name, every atom a screen draws and the element behind it, custom themes as small DefaultTheme subclasses, and per-element patches without a class at all.'
keywords: ['themes', 'dark mode', 'light mode', 'palette', 'ansi', 'elements', 'anatomy', 'atoms']
...
The page runs in that order: pick a shipped theme, see [every piece one draws](#what-a-theme-draws) and [the element behind it](#elements-what-a-theme-actually-implements), then [write your own](#writing-a-theme) or [patch a handful of glyphs](#patching-an-element). Why the division of labor falls where it does is the [specification](/specification#theme).
...
An **atom** is one named piece of that interface - the smallest thing worth naming on its own, and the unit a theme restyles. Every atom is drawn by an **element**, the method a theme answers with, and [Elements](#elements-what-a-theme-actually-implements) below lists all of them - not just a field's, but every block's.
...
| 2   | breadcrumb           | The trail of panel titles: every panel you've descended through, plus the one you're in.                              | `Form::create('Orchard')`, `->panel('Delivery', ...)`  | `breadcrumbLabel()`       |
...
| 2   | label           | The field's name. Every row has one.                                                                                    | `$p->select('Basket contents')`                   | `fieldLabel()`            |
...
`ChromeElementsInterface` is the one interface named for something other than a block, because what it draws belongs to no block: the frame surrounding every region at once, the mark saying a region's contents outran it, the gutter every block that comes and goes with the answers steps in behind, and the air between two things drawn side by side. The [specification](/specification#a-block-declares-the-elements-it-needs) has why that follows from the model.
...
To restyle one piece outright rather than recolor a family, override its **element** - the public method the block asks for. [Elements](#elements-what-a-theme-actually-implements) above lists every one of them, grouped by the block that declares it:
