...
Every string the TUI shows can be presented in another language - both the library's own **chrome** (key hints, buttons, validation and error messages) and the **questions** you declare (field and panel labels, descriptions, option labels and help text). A missing translation always falls back to the English source, so a partial catalog is safe to ship.
