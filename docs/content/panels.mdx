...
$form = Form::create('Order')
  ->panel('Basket', function (PanelBuilder $p): void {
    ...
    $p->confirm('Ripe only?')->default(TRUE);
    $p->select('Fruit')->default('apple')->options(['apple' => 'Apple', 'banana' => 'Banana', 'cherry' => 'Cherry']);

    // Full-screen editor, a better fit for the month grid.
    $p->calendar('Harvest date')->default('2026-07-15')->standalone();
  });
```
...
$form = Form::create('Basket settings')
  ...
  ->fixup(new Fixup(set: 'organic_only', to: FALSE, when: new Condition('grade', ne: 'premium')))
  ->panel('Basket', function (PanelBuilder $p): void {
    $p->select('Grade')->default('standard')->option('premium', 'Premium', 'Hand-picked')->option('standard', 'Standard', 'Everyday quality')->option('budget', 'Budget', 'Value pack');
    $p->confirm('Organic only?')->default(TRUE);

    $p->panel('Extras', function (PanelBuilder $sp): void {
      $sp->select('Add extras')->multiple()->options(['herbs' => 'Herbs', 'nuts' => 'Nuts', 'seeds' => 'Seeds']);

      $sp->panel('Portion', function (PanelBuilder $tp): void {
        $tp->suggest('Serving size')->default('medium')->options(['small' => 'Small', 'medium' => 'Medium', 'large' => 'Large']);
      });
    });
  });
```
...
$form = Form::create('Produce order')
  ->panel('Basket', function (PanelBuilder $p): void {
    ...
    $p->panel('Certification', function (PanelBuilder $sp): void {
      ...
      $sp->text('Certifier')->default('Soil Board');
      $sp->calendar('Renewed on')->default('2026-07-15');
    });
  });
```
...
$form = Form::create('New order')
  ...
  ->panel('Basics', function (PanelBuilder $p): void {
    $p->text('Produce name')->default('Pear')->required();
    $p->select('Category')->default('fruit')->options(['fruit' => 'Fruit', 'vegetable' => 'Vegetable', 'herb' => 'Herb']);
    $p->number('Quantity')->default(6)->min(1)->max(99);
  })
  ->panel('Delivery', function (PanelBuilder $p): void {
    $p->select('Ripeness')->default('ripe')->option('ripe', 'Ripe')->option('unripe', 'Unripe');
    $p->confirm('Express delivery?')->default(FALSE);
  });
...
$form = Form::create('Market stall')
  ...
  ->panel('Produce', function (PanelBuilder $p): void {
    ...
    $p->panel('Fruit', function (PanelBuilder $sp): void {
      $sp->select('Fruit')->default('apple')->options(['apple' => 'Apple', 'banana' => 'Banana', 'cherry' => 'Cherry']);
    });
    $p->panel('Vegetables', function (PanelBuilder $sp): void {
      $sp->select('Vegetables')->multiple()->default(['carrot'])->options(['carrot' => 'Carrot', 'tomato' => 'Tomato', 'spinach' => 'Spinach']);
    });
  })
  ->panel('Delivery', function (PanelBuilder $p): void {
...
The arrows move spatially: <kbd>Left</kbd>/<kbd>Right</kbd> along a row, <kbd>Up</kbd>/<kbd>Down</kbd> between rows, and off the grid onto whatever is drawn past it - the panel's own rows above or below the windows first, and the buttons after those. Where nothing at all is drawn past it the move stops, because walking the row the cursor is on is what <kbd>Left</kbd>/<kbd>Right</kbd> are for. <kbd>Enter</kbd> drills into the focused panel exactly as in the row list - the panel then takes the whole frame rather than the cell it was previewed in - and fields declared on the same panel keep their normal rows, above the grid or below it depending on which side of the sub-panels they were written. The declaration is refused where it is written - a grid needs at least one visual row, every visual row must hold at least one window, and the slots must exactly cover the sub-panels - so a mismatch throws at declaration time, never mid-session.
