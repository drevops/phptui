...
Given a **name** instead, the same `PanelBuilder::layout()` picks a layout that arranges the panel's own blocks into named regions - `$p->layout('two-column')`, then `$p->in('left')`. Both forms pick the layout a panel is arranged by, which is why mixing them in one call throws: a panel is arranged by one. Names are covered on [Layouts](/layouts).
