...
A **contextual help footer** runs along the bottom of every screen, listing exactly the keys that work right now and updating as focus moves between the hub and each editor. The hub shows move, select, back, quit and <kbd>?</kbd>. Each field adds its own bindings on top of accept/cancel: a select adds move, a multiple select adds toggle and select-all/none, a bounded number adds the step keys, a textarea adds newline and the external-editor handoff, a revealable password adds the reveal toggle. Press <kbd>?</kbd> at the hub and a fuller overlay lists the hub keys and every field type the form uses - so the form teaches its own less-common fields. The footer follows the active theme and key map (it degrades to ASCII glyphs and always reflects remapped keys), and the facade's `->footer(FALSE)` turns it off.
