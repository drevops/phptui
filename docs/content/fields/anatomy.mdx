...
|                               | `actionRule()`          | the line setting the buttons apart           |
...
So do the rules above and below them. `actionRule()` draws the line in the frame's own border style and at the width the theme lays content out to, so the buttons read as a compartment of the box rather than as rows with a rule of their own - and answers with an empty string on a frame drawn without a border, which has no line to lend them. They are rows of the block rather than something drawn around it, so what the buttons come to counts them in and no row is pushed off the bottom of the frame by a line nobody measured.
