...
Four rules are enforced when the form is built, each raising a `FormException`: a pattern declares at least one slot, no slot name is repeated, two slots are separated by some fixed text, and `->slot()` names a slot the pattern actually declares. The middle two answer to the same reason - every slot is filled in separately and the answer is read back off the assembled string - but they are separate rules, and a pattern can break either one on its own: `{{a}}-{{a}}` has a boundary and still has no one place to put a value, while `{{a}}{{b}}` names two slots and still has no boundary to read back at.
