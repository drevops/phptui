...
Four rules are enforced when the form is built, each raising a `FormException`: a pattern declares at least one slot, no slot name is repeated, two slots are separated by some fixed text, and `->slot()` names a slot the pattern actually declares. The middle two are the same requirement seen twice - every slot is filled in separately and the answer is read back off the assembled string, so `{{a}}{{a}}` has no one place to put a value and `{{a}}{{b}}` has no boundary to read back at.
