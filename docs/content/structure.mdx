...
final class DefaultLayout extends AbstractLayout {
  ...
  public function __construct() {
    ...
    $this->region('content', scrolls: TRUE);
...
$this->region('left', scrolls: TRUE);
$this->region('right', scrolls: TRUE);
```
...
$this->region('top', weight: 1, scrolls: TRUE);
$this->region('bottom', weight: 1, scrolls: TRUE);
```
...
$this->region('top', weight: 30, scrolls: TRUE);
$this->region('middle', weight: 40, scrolls: TRUE);
$this->region('bottom', weight: 30, scrolls: TRUE);
```
...
**Flow is not arrangement.** A *layout* apportions space between named areas that can be addressed, weighted and scrolled independently - that's *Arrange*, and only a layout claims it. A flow does none of that: blocks take their natural size and sit in the order they were added. It's the difference between a grid and the text flow inside one of its cells, and you don't nest a grid to put two words on one line.

So nesting a *layout* is for when you need what a flow can't give:

| You need | Use |
|---|---|
| Two blocks side by side | a flow |
| Areas you can address by name | a layout |
| Areas at declared sizes or weights | a layout |
