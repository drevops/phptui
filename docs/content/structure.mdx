...
final class DefaultLayout extends AbstractLayout {
  ...
  public function __construct() {
    ...
    $this->region('header')->fixed(1)->add(new Breadcrumb());
    $this->region('content')->scrolls();
    $this->region('footer')->fixed(1)->add(new Legend());
  }
...
| Name | Axis | Regions |
|---|---|---|
| `default` | rows | `header` (fixed 1, holds a Breadcrumb), `content` (scrolls), `footer` (fixed 1, holds a Legend) |
...
A layout can arrive with blocks already in its regions, and `DefaultLayout` does - a *breadcrumb* in the header, a *legend* in the footer. That's still a layout, not a new kind of thing. It's a **furnished** one.

| | Declares regions | Places blocks |
|---|---|---|
| `TwoColumnLayout` | ✓ | |
| `DefaultLayout` | ✓ | ✓ |

The furniture can't be a separate thing you bolt on, and the reason is worth knowing: **a block is placed by name, and only the layout knows which names exist.** A breadcrumb goes in `header` because `DefaultLayout` declared a `header`. A `TwoColumnLayout` has `left` and `right`, so the same furniture would have nowhere to go.

So whatever declares a region is what furnishes it, and a furnished layout is the honest name for a layout that does both.

Furnishing is only a default. Anything the layout placed can be moved or joined:

```php
$layout = new DefaultLayout();

// Keep the breadcrumb, and add a standing warning beside it.
...
```php
// A standing warning in the header, beside the breadcrumb already there.
$layout = new DefaultLayout();
...
```php
// DefaultLayout arrives furnished: a Breadcrumb in the header,
// a Legend in the footer. Only the panel and its actions are added.
$layout = new DefaultLayout();
