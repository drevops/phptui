...
The **screen** is the root. It holds one *layout* and claims one capability of its own - *Occupy*:
...
You build one by adding regions to it, and a region takes its blocks the same way. Two levels, one verb:

```php
(new Layout(Axis::Rows))
  ->add((new Region('header'))->add(new Breadcrumb()));
```
...
```php
(new Layout(Axis::Rows))
  ->add(new Region('header'))
  ->add(new Region('content', scrolls: TRUE))
  ->add(new Region('footer'));
```
...
```php
(new Layout(Axis::Columns))
  ->add(new Region('left', scrolls: TRUE))
  ->add(new Region('right'));
```

### Proportional columns

Columns divide the width on a twelve-unit grid. A region declares how many units it takes, so `6` and `6` splits evenly and `4` and `8` gives a third to one and two thirds to the other:

```php
(new Layout(Axis::Columns))
  ->add(new Region('left', span: 4))
  ->add(new Region('right', span: 8));
```
...
Omit the spans and the width divides evenly, so two regions are `6` and `6` and three are `4`, `4` and `4`. Twelve is chosen because it divides by two, three, four and six, which covers every split you'd reach for without fractions.

Spans belong to the *columns* axis only. On the *rows* axis a region is as tall as its blocks, and the region that *Scrolls* takes whatever height is left - which is why a header and footer sit tight against their content while the middle stretches.
...
```php
// The Panel in 'content' carries a Layout(Axis::Columns) of its own.
(new Layout(Axis::Rows))
  ->add(new Region('header'))
  ->add((new Region('content', scrolls: TRUE))->add($panel))
  ->add(new Region('footer'));
```
...
| `two-column` | columns | `left` (span 6), `right` (span 6) |
...
$layout = (new Layout(Axis::Rows))
  ->add((new Region('header'))->add(new Breadcrumb()))
  ->add((new Region('content', scrolls: TRUE))->add($panel)->add(new Buttons()))
  ->add((new Region('footer'))->add(new KeyHints()));
