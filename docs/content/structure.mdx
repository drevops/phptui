...
| Layout | `default`, `plain`, `two-column` | the screen's `default` |
...
| **Bind** | It owns keystrokes while it's in play, and says which. |
...
| **Reject** | It can refuse what you gave it, and say why. |
...
| | Act | Answer | Arrange | Bind | Constrain | Contain | Depend | Enter | Expand | Fill | Float | Focus | Reject | Reuse | Scroll | Show |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Screen | | | | | | ✓ | | | | ✓ | | | | | | |
| Layout | | | ✓ | | | ✓ | | | | | | | | ✓ | | |
| Region | | | | | | ✓ | | | | | | | | | ✓ | |
...
A **layout** is composed of *region* instances and decides where they sit. It claims *Arrange*, *Contain* and *Reuse*.
...
The degenerate case, and a useful one: no chrome, just the content. It needs no special axis - it's `rows()` with a single region:
...
*Reuse* means a layout is named and reusable, so the same one serves a *screen* and a *panel*. Three ship, and a form picks one rather than describing an arrangement inline:

| Name | Axis | Regions |
|---|---|---|
| `default` | rows | `header`, `content` (scrolls), `footer` |
| `plain` | rows | `content` (scrolls) |
...
└─ Bind       it owns the keystrokes while you're in it, and its keys fill the legend
```
