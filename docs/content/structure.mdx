...
$this->region('top')->weight(1)->scrolls();
$this->region('bottom')->weight(1)->scrolls();
```
...
$this->region('content')->weight(1);  // a share of whatever is left
```
...
$this->region('top')->weight(30)->scrolls();
$this->region('middle')->weight(40)->scrolls();
$this->region('bottom')->weight(30)->scrolls();
```
...
final class SidebarLayout extends AbstractLayout {
  ...
  public function __construct() {
    ...
    $this->region('main')->weight(1)->scrolls();
  }
...
| `->weight(2)` | twice the share of the remainder that `weight(1)` gets |
...
`fixed()` and `weight()` are the odd pair out: they *declare* a size but don't compute one, because *Arrange* is the layout's. The region states what it wants and the layout does the arithmetic.
...
**Flow is what a layout and a region share; sizing is what only a layout does.** Both run their contents in one direction, so both claim *Flow*. Only a layout also apportions space between them - naming the areas, weighting them, letting each scroll on its own - and that's *Arrange*. A region's blocks take their natural size and sit in the order they were added.
