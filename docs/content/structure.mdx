...
(new Layout(Axis::Rows))
  ->region('header', fixed: 1)
  ->region('content', scrolls: TRUE)
  ->region('footer', fixed: 1);
```
...
```php
(new Layout(Axis::Rows))
  ->region('top', weight: 1, scrolls: TRUE)
  ->region('bottom', weight: 1, scrolls: TRUE);
```
...
```php
->region('header', fixed: 1)      // exactly one row (or column)
->region('content', weight: 1)    // a share of whatever is left
```
...
**`weight` is a proportion of the remainder.** Weights don't sum to anything in particular, so `30`, `40`, `30` and `3`, `4`, `3` mean the same thing:

```php
(new Layout(Axis::Rows))
  ->region('top', weight: 30, scrolls: TRUE)
  ->region('middle', weight: 40, scrolls: TRUE)
  ->region('bottom', weight: 30, scrolls: TRUE);
```
...
Declaring neither is `weight: 1`, which is why the default layout can leave its middle region bare:
...
The two mix without negotiating: fixed regions are subtracted first, then what remains is divided by the weights. A header stays one row however tall the terminal, and the weighted regions share the rest between them.

### Who calculates what

Sizing and scrolling are two calculations, and each can only be done by one level.

**The layout sizes.** "How many rows does `content` get" can't be answered by `content` alone - its fixed siblings have to come off the top first, and the remainder split by weight. Only the thing that sees every region can do that arithmetic, which is why *Arrange* is the layout's capability.
...
Character cells don't divide evenly, so the layout rounds: it takes the fixed sizes off, divides the remainder by the weights, and hands any leftover cell to the last weighted region. A region never sees that arithmetic - it's told a number and gets on with it.
...
A layout is a class, because a layout *does* something - it works out how big each region is. A subclass inherits that arithmetic and declares its regions:
...
final class SidebarLayout extends Layout {
  ...
  public function __construct() {
    ...
    $this->region('main', weight: 1, scrolls: TRUE);
  }
...
Subclassing earns its keep when a layout arranges *differently* rather than merely declaring different regions - one that packs regions to fit, or gives the focused region extra room. A layout that only names regions can skip the class and be built inline.
...
$columns = (new Layout(Axis::Columns))
  ->region('left', weight: 1)
  ->region('right', weight: 2);
