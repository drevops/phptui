...
Every block claims *Show*, and only a *panel* nests anything - and what it nests is a *layout*, never blocks directly. So no block ever contains a field or another block. Seven kinds exist:
...
- **Legend** - renders the keys that apply right now, rewriting itself as focus moves.
- **Actions** - the buttons that end the form: submit, cancel, and any the form declares. Withholds the submit with a message while a required field is empty.
...
A block names its elements in an interface, and a theme implements it:
...
*Breadcrumb* and *Legend* are the smallest complete examples, so here they are end to end - the elements each declares, and what each one draws.

**Breadcrumb** needs two:
...
**Legend** needs three, and composes them per key:
...
Both follow the same shape: an element per distinct thing the block styles, prefixed with the block's name so a theme can implement every interface on one class without collisions.
