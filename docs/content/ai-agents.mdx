...
Every prompt carries every key, whether or not the field declares it - so a reader never has to tell a missing key from an unset one. A note and a progress row are absent: they collect no answer, so they are not prompts anything drives or validates.
