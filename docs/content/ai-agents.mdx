...
"crop": {
  ...
  "x-hint": "Type a few letters to filter.",
...
2. **Environment** - the per-question variable named in `env` (e.g. `TUI_NAME`), the uppercased id under the active prefix, or any name listed in `x-env-aliases` when that one is unset.
